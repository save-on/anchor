...
struct ElementData {
  GstElement *audio_source, *video_source, *video_queue, *audio_queue,
      *audio_convert, *video_convert, *audio_encoder, *video_encoder, *mux,
      *sink;
};
...
int main(int argc, char *argv[]) {
  ...
  element.video_encoder =
      gst_element_factory_make("nvh264enc", "video_encoder");
  element.audio_encoder =
      gst_element_factory_make("lamemp3enc", "audio_encoder");
  ...
  gst_bin_add_many(GST_BIN(pipeline), element.video_source,
                   element.video_convert, element.video_encoder,
                   element.video_queue, element.audio_source,
                   element.audio_convert, element.audio_encoder,
                   element.audio_queue, element.mux, element.sink, NULL);

  if (gst_element_link(element.mux, element.sink) != TRUE ||
      gst_element_link_many(element.video_source, element.video_convert,
                            element.video_encoder, element.video_queue,
                            NULL) != TRUE ||
      gst_element_link_many(element.audio_source, element.audio_convert,
                            element.audio_encoder, element.audio_queue,
                            NULL) != TRUE) {
  ...
  std::cout << "Obtained request pad " << gst_pad_get_name(pad.mux_audio)
            << " audio branch\n";
  ...
  std::cout << "Obtained request pad " << gst_pad_get_name(pad.mux_video)
            << " video branch\n";
